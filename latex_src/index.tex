...
\hangindent=2pc \hangafter=1 \verb'\' backslash (反斜杆) 28, 30, 41, 43
...
\hangindent=2pc \hangafter=1 \verb'>' output redirection (输入重定向) 56, 58, 188

\hangindent=2pc \hangafter=1 \verb'>>' output redirection (输入重定向) 56, 58, 188

\hangindent=2pc \hangafter=1 \verb'|' output redirection (输入重定向) 58, 188

\hangindent=2pc \hangafter=1 \verb"' '" quotes (\verb'单引号') 2, 4-5, 65, 100
...
\hangindent=2pc \hangafter=1 \verb'$0', side-effects on (副作用) 36 43

\hangindent=2pc \hangafter=1 \verb'\007' bell character (响铃符) 31

\hangindent=2pc \hangafter=1 \verb'\b' backspace character (退格符) 31

\hangindent=2pc \hangafter=1 \verb'/dev/tty' file (\verb'/dev/tty' 文件) 59

\hangindent=2pc \hangafter=1 \verb'-f' option (\verb'-f' 选项) 4-5, 63, 65, 187

\hangindent=2pc \hangafter=1 \verb'-F' option (\verb'-F' 选项) 60, 63, 187
...
\hangindent=2pc \hangafter=1 \verb'$'\textit{n} field (\verb'$n'\textit{n}
字段) 5, 35
...
\hangindent=2pc \hangafter=1 arithmetic expression grammar (算法表达式语法) 145
...
\hangindent=2pc \hangafter=1 arithmetic operator, table of (算术运算符表格) 46
...
\hangindent=2pc \hangafter=1 assignment, multiple (复合赋值) 39
...
\hangindent=2pc \hangafter=1 backslash, \verb'\' (反斜杆) 28, 30, 41, 43

\hangindent=2pc \hangafter=1 backspace character, \verb'\b' (退格符) 31

% TODO "bailing out" 不知道该怎么翻译
\hangindent=2pc \hangafter=1 bailing out 4

\hangindent=2pc \hangafter=1 balanced delimiters (均衡的分隔符) 77, 195

\hangindent=2pc \hangafter=1 base table (基表) 106

\hangindent=2pc \hangafter=1 batch sort test program (批处理的排序测试程序) 155

\hangindent=2pc \hangafter=1 \verb'BEGIN' and \verb'END', multiple (多个的
...
\hangindent=2pc \hangafter=1 built-in variables, table of (内建变量表格) 36
...
\hangindent=2pc \hangafter=1 characters, table of escape (转义字符表格) 31
...
\hangindent=2pc \hangafter=1 comparison operators, table of (比较运算符表格) 25
...
\hangindent=2pc \hangafter=1 compiler model (编译器型) 131
...
\hangindent=2pc \hangafter=1 continuation after \verb',' (延续) 22, 188
...
\hangindent=2pc \hangafter=1 error, syntax (语法的错误) 4

\hangindent=2pc \hangafter=1 escape sequence (转义序列) 31, 35, 191

\hangindent=2pc \hangafter=1 escape sequences, table of (转义序列表格) 31
...
\hangindent=2pc \hangafter=1 expressions, primary (主表达式) 34
...
\hangindent=2pc \hangafter=1 form letters (套用信函) 100
...
\hangindent=2pc \hangafter=1 grammar, contex-free 113, 145, 148

\hangindent=2pc \hangafter=1 \verb'grap' language 139, 152

\hangindent=2pc \hangafter=1 graph, constraint 170

\hangindent=2pc \hangafter=1 graph cycle 171, 173-174, 177

\hangindent=2pc \hangafter=1 graph, dependency 176

\hangindent=2pc \hangafter=1 \verb'graph' language 135

\hangindent=2pc \hangafter=1 \verb'graph' program 137

\hangindent=2pc \hangafter=1 \verb'grep' command v, 181, 184
...
\hangindent=2pc \hangafter=1 \verb'gsub' function 42, 71, 101, 119, 122, 182
...
\hangindent=2pc \hangafter=1 letters, form (套用信函) 100
...
\hangindent=2pc \hangafter=1 little languages (小型语言) iv-v, 131, 152, 156, 159
...
\hangindent=2pc \hangafter=1 multiple assignment (复合赋值) 39

\hangindent=2pc \hangafter=1 multiple \verb'BEGIN' and \verb'END' (多个
\verb'BEGIN' 与 \verb'END') 23, 169
...
\hangindent=2pc \hangafter=1 operators, table of arithmetic (算术运算符表格) 46

\hangindent=2pc \hangafter=1 operators, table of comparison (比较运算符表格) 25
...
\hangindent=2pc \hangafter=1 patterns, table of (模式表格) 33
...
\hangindent=2pc \hangafter=1 \verb'pic' language (语言 \verb'pic') 139
...
\hangindent=2pc \hangafter=1 Postscript language (语言 Postscript) 142
...
\hangindent=2pc \hangafter=1 primary expressions (主表达式) 34
...
\hangindent=2pc \hangafter=1 printing a blank line (打印一个空行) II, 55
...
\hangindent=2pc \hangafter=1 quadratic algorithm (二次方程序) 157,162, 183
...
\hangindent=2pc \hangafter=1 range pattern (随机模式) 32, 85, 187
...
\hangindent=2pc \hangafter=1 regular expressions, table of (正则表达式表格) 32
...
\hangindent=2pc \hangafter=1 scalar parameter (标题参数) 54
...
\hangindent=2pc \hangafter=1 string functions, table of (字符串函数表格) 42
...
\hangindent=2pc \hangafter=1 TEX formatter (排序测试 TEX) 120,124
...
\hangindent=2pc \hangafter=1 topological sort algorithm (拓扑排序程序) 171
