...
当你在阅读这本书时, 请尝试运行并修改书中的程序. 由于大多数程序都很简短,
通过这种方式你可以快速理解 awk 的工作方式. 在 Unix 系统中, 可以这样运行
上面提到的两个示例程序:
\begin{myverb}
    $ awk '$3 > 0 { print $1, $2 * $3 }' emp.data
    Kathy 40
...
\marginpar{4}
\begin{pattern}
    \indent\verb"$ awk '$3 == 0 { print $1 print $1 }'"\par
...
\subsection{{\bfseries\texttt{NF}}, 字段的数量}
...
\begin{myverb}
    ...
    Susie   4.25    18
\end{myverb}
你可以在同一行放置多个语句, 语句之间用分号分开. 注意 \verb'print ""' 打
印一个空行, 它与一个单独的 \print 并不相同, 后者打印当前行.

\section{用 AWK 计算}
\label{sec:computing_with_awk}

一个动作就是一个语句序列, 语句之间用分号或换行符分开. 你已经见过只有一条
单独的 \print 语句的动作. 这一小节提供的例子所包含的语句可以用来进行简单
的数学或字符串计算. 在这些语句里, 你不仅可以使用内建变量, 比如 \nf, 还
...
\subsection{\textbf{\texttt{If-Else}} 语句}
...
\subsection{\textbf{\texttt{While}} 语句}
...
问题. 你还是应该浏览整本书; 每一个例子都传达出一些关于 awk 的知识点, 可能是
