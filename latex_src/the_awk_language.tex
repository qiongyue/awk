...
\subsection*{输入文件\textbf{\filename{countries}}}
...
{\CJKfamily{heiti}{常量}} (\emph{Constants}). Awk 中只有两种类型的常量: 字符串与数值.
将一个
...
{\CJKfamily{heiti}{变量}} (\emph{Variables}). 表达式可以包含若干种类型的变量: 用户定义的,
...
{\CJKfamily{heiti}{内建变量}} (\emph{Built-In Variables}). 表 \ref{tbl:built_in_variables}
...
{\CJKfamily{heiti}{字段变量}} (\emph{Field Variables}). 当前输入行的字段从 \verb'$1',
...
{\CJKfamily{heiti}{算术运算符}} (\emph{Arithmetic Operators}). Awk 提供了通常的 \verb'+',
...
{\CJKfamily{heiti}{比较运算符}} (\emph{Comparison Operators}). 比较表达式指的是那些含有
...
{\CJKfamily{heiti}{逻辑运算符}} (\emph{Logical Operators}). 逻辑运算符将多个表达式组合
...
{\CJKfamily{heiti}{条件表达式}} (\emph{Conditional Expressions}). 一个条件表达式具有形式:
\begin{pattern}
...
{\CJKfamily{heiti}{赋值运算符}} (\emph{Assignment Operators}). 在赋值表达式中可以使用 7
种赋值运算符. 最简单的赋值表达式是
\begin{pattern}
...
{\CJKfamily{heiti}{自增与自减}} (\emph{Increment and Decrement Operators}). 赋值语句:
\begin{awkcode}
    n = n + 1
\end{awkcode}
...
{\CJKfamily{heiti}{内建算术函数}} (\emph{Built-In Arithmetic Functions}). 内建算术函数在
...
{\CJKfamily{heiti}{字符串运算符}} (\emph{String Operators}). Awk 中只有一种字符运算符 ---
...
{\CJKfamily{heiti}{作为正则表达式的字符串}} (\emph{Strings as Regular Expressions}).
...
{\CJKfamily{heiti}{内建字符串函数}} (\emph{Built-In String Functions}). 表
...
{\CJKfamily{heiti}{数值还是字符串}} (\emph{Number or String}). 表达式的值可以自动从
...
\begin{awkcode}
    $1 "" == $2
\end{awkcode}
为了让比较强制按照数值类型来进行, 我们需要将\cterm{两个}字段都转换成数值:
\begin{awkcode}
    $1 + 0 == $2 + 0
\end{awkcode}
无论字段包含什么内容, 这个方式总能奏效.
...
{\CJKfamily{heiti}{运算符的总结}} (\emph{Summary of Operators}). 可以出现在表达式中的运算
...
原因, awk 的数组称为 \cterm{关联数组} (\emph{associative arrays}).
...
{\CJKfamily{heiti}{\textbf{\texttt{delete}} 语句}} (\emph{The \texttt{delete} Statement}).
一个数组元素可以通过
\begin{pattern}
...
{\CJKfamily{heiti}{\textbf{\texttt{split}} 函数}} (\emph{The \texttt{split} Function}). 函数
...
{\CJKfamily{heiti}{多维数组}} (\emph{Multidimensional Arrays}). Awk 不直接支持多维数组,
...
任何函数体内. 每一个对函数的使用都叫做一个 \cterm{调用} (\emph{call}).
...
它们与程序中其他同名的变量没有任何关联. 但是, \cterm{其他所有的变量都是全
...
\begin{awkcode}
    field1
      field1
        field1      field2
\end{awkcode}
然而, 当 \verb'FS' 是其他值时, 前导的空格与制表符\cterm{不会}被丢弃.
