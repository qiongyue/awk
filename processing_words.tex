...
进行格式化, 则最终的排版结果是
\begin{pattern}
\indent\indent    arithmetic operators 36, 44\par
\indent\indent    coercion rules 44\par
\indent\indent    coercion to number 45\par
\indent\indent    coercion to string 45\par
\indent\indent    comparison, numeric 44\par
\indent\indent    comparison, string 44\par
\indent\indent    control-flow statements 48\par
\indent\indent    \texttt{FS} variable 35, 36, 52\par
\indent\indent    \texttt{if-else} statement 47\par
\indent\indent    number, coercion to 45\par
\indent\indent    numeric comparison 44\par
\end{pattern}
...
\begin{exercise}
    修改或增强索引程序的功能, 使得它可以提供分层的索引 (\textit{See} 与
...
\begin{exercise}
    构造一个准备单词, 短语列表的工具程序, 利用该程序来自动创建索引.
