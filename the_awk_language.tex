...
这一章的第一节会详细讨论模式. 第二节通过描述表达式, 赋值语句与流程控制语句,
展开对动作的讨论. 剩下的小节包括函数定义, 输出, 输入, 以及 awk 如何调用其
...
在一个字符集合内, 所有的字符都具有它自身的字面意义, 除了引用字符 \verb'\',
...
\texttt{sub} 的返回值小于等于 1, 而 \texttt{gsub}
返回值大于等于 0. --- 译者}.
\marginpar{43}
...
\verb'while' 与 \verb'do' 测试表达式, 或 \verb'for' 的 \expr$_3$.
...
\begin{pattern}
    \verb'for ('\textit{variable} \verb'in' \textit{array}\verb')'
        \stmt
\end{pattern}
...
\begin{center}
        \begin{tabular}{l|l|l}
            \hline
            \hline
            % TODO: the first line should be centered.
            \texttt{fmt}  & \verb'$1'     &
            \verb'printf(fmt, $1)' \\
            \hline
...
这里来的话, 那你确实是非常专注. 到后面你就会发现, 时不时的就需要回到这章
再看一下某些小节, 有时候是为了精确的了解程序如何工作的, 有时候则是因为后面
的章节出现了一些你之前没有试过的构造方法.
